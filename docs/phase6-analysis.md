...
| 问题 | 答案 |
|------|------|
| token_loss_top20 在真实数据上能检测噪音吗？ | ⚠️ 受长度偏置严重影响，短响应产生虚假高信号 |
| 信号在受控实验和自然数据上的行为一致吗？ | ❌ 差异显著——受控数据的双峰分布不适用于自然数据 |
| 最有价值的发现是什么？ | **响应长度偏置**——这是之前被忽略的重要方法论限制 |
| Phase 6 算成功还是失败？ | 有价值的负面结果——定义了信号的边界，为后续研究提供方法论改进方向 |
